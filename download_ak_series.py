...
def send_file_to_telegram(
    ...
    *,
    ...
    large_file_mode: str = "link",
) -> None:
    if large_file_mode == "link" and file_path.stat().st_size > TELEGRAM_BOT_FILE_LIMIT_BYTES:
        notice = (
            f"{caption}\n"
            f"الحجم كبير على Telegram Bot API (أكبر من 50MB)."
        )
        if fallback_url:
            notice += f"\nرابط التحميل:\n{fallback_url}"
        send_text_to_telegram(session, bot_token, chat_id, notice)
        return
    ...
    with file_path.open("rb") as f:
        try:
            ...
            if code == 413 and large_file_mode == "link":
                notice = (
                    f"{caption}\n"
                    f"تعذر الرفع بسبب الحجم (413 Request Entity Too Large)."
                )
                if fallback_url:
                    notice += f"\nرابط التحميل:\n{fallback_url}"
                send_text_to_telegram(session, bot_token, chat_id, notice)
...
def send_text_to_telegram(
    ...
    resp = session.post(api_url, data={"chat_id": chat_id, "text": text[:4096]}, timeout=90)
    resp.raise_for_status()
    payload = resp.json()
    if not payload.get("ok"):
        raise RuntimeError(f"Telegram API error: {payload}")
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        choices=["link", "fail"],
        default="link",
        help='Behavior when Telegram rejects big files: "link" (default) or "fail".',
    )
    ...
    if telegram_enabled:
        ...
        print(f"Telegram large-file mode: {args.telegram_large_mode}")
                ...
                if telegram_enabled and args.telegram_send_existing and not args.dry_run:
                    try:
                        ...
                        send_file_to_telegram(
                            ...
                            existing_file,
                            caption,
                            large_file_mode=args.telegram_large_mode,
                        )
